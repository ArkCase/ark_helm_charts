...
{{- define "__arkcase.box.license.compute" -}}
  {{- $ctx := $ -}}
  {{- if not (include "arkcase.isRootContext" $ctx) -}}
    {{- fail "The parameter must be the root context ($ or .)" -}}
  {{- end -}}
  ...
  {{- $result := dict -}}
  {{- $license := (include "arkcase.license" (dict "ctx" $ctx "name" "box") | fromYaml) -}}
  {{- if and $license $license.data -}}
    ...
    {{- end -}}
    {{- $result = set $result "box-private-key.rsa" $license -}}
  {{- end -}}
  {{- $result | toYaml -}}
{{- end -}}

{{- define "arkcase.box.license.secret.name" -}}
  {{ $ctx := $ -}}
  {{- if not (include "arkcase.isRootContext" $ctx) -}}
    {{- fail "Must include the root context as the only parameter" -}}
  {{- end -}}
  {{- printf "%s-box-license" $.Release.Name -}}
{{- end -}}

{{- define "arkcase.core.box.license.volume" -}}
  {{- $ctx := $ -}}
  {{- if not (include "arkcase.isRootContext" $ctx) -}}
    {{- fail "The parameter must be the root context ($ or .)" -}}
  {{- end -}}
  {{- $license := (include "__arkcase.box.license.compute" $ctx | fromYaml) -}}
  {{- if $license -}}
# Box RSA Key volume begins
- name: "box-license"
  secret:
    optional: false
    secretName: {{ include "arkcase.box.license.secret.name" $ctx | quote }}
    defaultMode: 0444
    items:
      - key: "box-private-key.rsa"
        path: "box-private-key.rsa"
# Box RSA Key volume ends
  {{- end -}}
{{- end -}}

{{- define "arkcase.core.box.license.volumeMount" -}}
  {{- $ctx := $ -}}
  {{- if not (include "arkcase.isRootContext" $ctx) -}}
    {{- fail "The parameter must be the root context ($ or .)" -}}
  {{- end -}}
  ...
  {{- $license := (include "__arkcase.box.license.compute" $ctx | fromYaml) -}}
  {{- if $license -}}
# Box RSA Key mount begins
- name: "box-license"
  mountPath: {{ printf "%s/%s" $mountPath "box-private-key.rsa" | quote }}
  subPath: "box-private-key.rsa"
  readOnly: true
# Box RSA Key mount ends
  {{- end -}}
{{- end -}}
