...
ALIAS_ARGS=("${INSTANCE}" "${MINIO_ADMIN_URL}" "${MINIO_ROOT_USER}" "${MINIO_ROOT_PASSWORD}")
mcli config host add "${ALIAS_ARGS[@]}" || mcli alias set "${ALIAS_ARGS[@]}" || fail "Failed to define the alias [${INTANCE}] for ${MINIO_ADMIN_URL} using either 'mcli config' or 'mcli alias'"
