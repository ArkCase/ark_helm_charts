{{- define "arkcase.migrator.objectName" -}}
...
{{- define "arkcase.migrator.acme" -}}
  ...
  {{- $serviceName := (include "arkcase.migrator.objectName" $ctx) -}}
...
{{- define "arkcase.migrator.content-env" -}}
  {{- $ctx := $ -}}
  {{- if not (include "arkcase.isRootContext" $ctx) -}}
    {{- fail "You must supply the root context as the only parameter" -}}
  {{- end -}}
